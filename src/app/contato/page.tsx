...
export default function ContatoPage() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="pt-28 pb-16 bg-gradient-to-b from-amber-50 to-white dark:from-amber-950/30 dark:to-slate-900">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <span className="inline-block px-4 py-1.5 rounded-full bg-amber-100 dark:bg-amber-900/50 text-amber-700 dark:text-amber-300 text-sm font-bold mb-4">
            CONTATO
          </span>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-8 shadow-xl border border-slate-100 dark:border-slate-800">
                ...
                <form onSubmit={handleSubmit} className="space-y-5">
                  <div>
                    ...
                    <input type="text" required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500 outline-none bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white" placeholder="Seu nome completo" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Email</label>
                    <input type="email" required className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500 outline-none bg-slate-50 dark:bg-slate-800/50 text-slate-900 dark:text-white" placeholder="seuemail@exemplo.com" />
                  </div>
                  <div>
                    <label className="block text-sm font-semibold text-slate-700 dark:text-slate-200 mb-1.5">Mensagem</label>
                    <textarea required rows={5} className="w-full px-4 py-3 rounded-xl border border-slate-200 dark:border-slate-700 focus:ring-2 focus:ring-emerald-500 outline-none bg-slate-50 dark:bg-slate-800/50 resize-none text-slate-900 dark:text-white" placeholder="Como podemos ajudar?" />
                  </div>
                  <button type="submit" className="w-full flex items-center justify-center gap-2 bg-gradient-to-r from-emerald-500 to-emerald-400 text-white py-4 rounded-xl font-bold text-lg hover:shadow-lg transition-all">
                    {sent ? <>✓ Mensagem Enviada!</> : <><Send className="w-5 h-5" /> Enviar Mensagem</>}
                  </button>
                </form>
              </div>
            </div>
            <div className="space-y-6">
              ...
              <div className="bg-white dark:bg-slate-900 rounded-2xl p-6 border border-slate-100 dark:border-slate-800 shadow-sm space-y-5">
                <div className="flex items-center gap-4">
                  <div className="w-10 h-10 rounded-lg bg-blue-50 dark:bg-blue-900/30 flex items-center justify-center"><Mail className="w-5 h-5 text-blue-600" /></div>
                  <div><h4 className="font-bold text-slate-900 dark:text-white text-sm">E-mail</h4><a href="mailto:contato@descubraobrasil.com" className="text-slate-600 dark:text-slate-300 text-sm hover:text-emerald-600">contato@descubraobrasil.com</a></div>
                </div>
                <div className="flex items-center gap-4">
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div><h4 className="font-bold text-slate-900 dark:text-white text-sm">Localização</h4><p className="text-slate-600 dark:text-slate-300 text-sm">Brasil, 100% Digital</p></div>
                </div>
              </div>
            </div>
          </div>
